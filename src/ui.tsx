  ...
  return <main onDragOver={e => { e.preventDefault(); }} onDrop={e => { e.preventDefault(); }}>
    ...
    {settingsOpen && <section className="panel settings"><div><h2>Ducking</h2>
      ...
      <div><h2>Monitoring & output</h2>
        ...
        <label>Music in my headphones: {Math.round(audio.settings.monitorMusicVolume * 100)}%<input type="range" min="0" max="1" step="0.01" value={audio.settings.monitorMusicVolume} onChange={e => change({ ...audio.settings, monitorMusicVolume: Number(e.target.value) })} /><small>Listeners always hear music at the Music/Master level; this only changes how loud it is for you.</small></label>
