...
contextBridge.exposeInMainWorld('micmix', {
  ...
  refreshDevices: ok,
  onReport: noop,
  getConfig: () => Promise.resolve({ setupDone: true, micLabel: 'Microphone (Blue Yeti)', monitorLabel: 'Headphones (Realtek(R) Audio)', updateCheck: true, fivemTune: true, closeToTray: true, unavailableHotkeys: [], hotkeys: { playPause: 'Ctrl+Alt+P', next: 'Ctrl+Alt+N', previous: 'Ctrl+Alt+B', muteMic: 'Ctrl+Alt+K', stopPads: 'Ctrl+Alt+X', live: 'Ctrl+Alt+L', show: 'Ctrl+Alt+H' }, appVersion: version }),
  saveDevices: ok,
  completeSetup: ok,
  assignPad: ok,
  setPadHotkey: ok,
