...
module.exports = async function smoke(ui) {
  ...
  await evalUi(`(() => { const sheet = document.querySelector('.sheet-body'); const el = document.querySelector('${label}'); sheet.scrollTop = el.getBoundingClientRect().top - sheet.getBoundingClientRect().top + sheet.scrollTop - sheet.clientHeight / 2; })()`);
  await sleep(500);
